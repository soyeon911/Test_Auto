...
def test_post__api_v2_mask_positive(base_url):
    """[rule:positive] Happy-path — valid request should succeed."""
    resp = requests.post(f"{base_url}/api/v2/mask", json={'image_data': 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='}, timeout=10)
...
def test_post__api_v2_mask_semantic_image_data_invalid_b64(base_url):
    """[rule:semantic_probe] 'image_data' tag=base64_image probe=invalid_b64."""
    resp = requests.post(f"{base_url}/api/v2/mask", json={'image_data': 'not_base64!@#'}, timeout=10)
...
def test_post__api_v2_mask_semantic_image_data_empty_b64(base_url):
    """[rule:semantic_probe] 'image_data' tag=base64_image probe=empty_b64."""
    resp = requests.post(f"{base_url}/api/v2/mask", json={'image_data': ''}, timeout=10)
    assert resp.status_code < 500, (
...
def test_post__api_v2_mask_semantic_image_data_null_value(base_url):
    """[rule:semantic_probe] 'image_data' tag=base64_image probe=null_value."""
    resp = requests.post(f"{base_url}/api/v2/mask", json={'image_data': None}, timeout=10)
    assert resp.status_code < 500, (
